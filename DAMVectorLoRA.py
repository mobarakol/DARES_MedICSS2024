...
class _VeRA_qkv(nn.Module):
    def __init__(self, w: nn.Module, linear_a: nn.Module, linear_b: nn.Module, d: nn.Module, b: nn.Module):
        super().__init__()
        self.w = w
    ...
    def forward(self, x):
        W = self.w(x)
        residual = W.clone()
        deltaW = self.b(self.linear_b(self.d(self.linear_a(x))))
...
class VeraInitializer:
    def __init__(self, model, r=8, vera=['q', 'v']):
        self.model = model
        self.r = r
        self.vera = vera
        self.w_As = []
        self.w_Bs = []
        self.w_ds = []  # New list for d matrices
        self.w_bs = []  # New list for b matrices
        self.initialize_vera()

    def initialize_vera(self):
        ...
        for t_layer_i, blk in enumerate(self.model.backbone.encoder.layer):
            ...
            if 'q' in self.vera:
                ...
                d_linear_q = nn.Linear(self.r, self.r, bias=False)  # New d matrix for q
                b_linear_q = nn.Linear(dim, dim, bias=False)  # New b matrix for q
                self.w_As.append(w_a_linear_q)
                self.w_Bs.append(w_b_linear_q)
                self.w_ds.append(d_linear_q)  # Add d matrix to list
                self.w_bs.append(b_linear_q)  # Add b matrix to list
                blk.attention.attention.query = _VeRA_qkv(w_q, w_a_linear_q, w_b_linear_q, d_linear_q, b_linear_q)



        self.reset_parameters()
        print("VeRA params initialized!")

    def reset_parameters(self):
            for w_A in self.w_As:
                nn.init.kaiming_uniform_(w_A.weight, a=math.sqrt(5))
                w_A.weight.requires_grad = False  # Freeze the weights

            for w_B in self.w_Bs:
                nn.init.kaiming_uniform_(w_B.weight, a=math.sqrt(5))
                w_B.weight.requires_grad = False  # Freeze the weights

            for w_d in self.w_ds:
                # initialize d matrices with only 1s
                nn.init.constant_(w_d.weight, 1)

            for w_b in self.w_bs:
                # initialize b matrices with only 0s
                nn.init.constant_(w_b.weight, 0)


class DARES_V(nn.Module):
    def __init__(self, enable_vera=True, r = 8, vera = ['q', 'v']):
        super(DARES_V, self).__init__()
        ...
        if enable_vera:
            self.vera_initializer = VeraInitializer(model, r, vera)
    ...
    def save_parameters(self, filename: str) -> None:
        ...
        save both vera and fc parameters.
        """
        ...
        print('saved vera parameters to %s.' % filename)
    ...
    def load_parameters(self, filename: str, device: str) -> None:
        ...
        load both vera and fc parameters.
        """
        ...
        print('loaded vera parameters from %s.' % filename)
...
model = DARES_V(enable_vera=True)
import DARES
model_orig = DARES.DARES(enable_lora=False)
pixel_values = torch.randn(1, 3, 256, 320)
outputs = model(pixel_values)
print('NEW', outputs[("disp", 0)])
outputs_orig = model_orig(pixel_values)
print('ORIG', outputs_orig[("disp", 0)])

print('DIFF', outputs[("disp", 0)] - outputs_orig[("disp", 0)])