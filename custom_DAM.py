...
class Customised_DAM(nn.Module):
    def __init__(self, r = 8, lora = ['q', 'v']):
        ...
        for t_layer_i, blk in enumerate(self.backbone.encoder.layer):
            ...
            #     dim = blk.attention.attention.key.in_features
            # if 'v' in self.lora
            w_v = blk.attention.attention.value
            #     dim = blk.attention.attention.value.in_features
            dim = blk.attention.attention.query.in_features
            ...
            blk.attention.attention.query = _LoRA_qkv(
                w_q,
                w_a_linear_q,
                w_b_linear_q
            )
            blk.attention.attention.value = _LoRA_qkv(
                w_v,
                w_a_linear_v,
                w_b_linear_v
            )
